...
function releaseRouteSplitText(proof) {
  const count = Number(proof.count ?? 0);
  const localRoutes = Number(proof.localRoutes ?? 0);
  const cloudRoutes = Number(proof.cloudRoutes ?? 0);
  if (count <= 0) return '';
  return ` It split ${localRoutes}/${count} routes to local models and ${cloudRoutes}/${count} routes to cloud models before any provider execution, so a cloud route means selected target rather than a live cloud call.`;
}
