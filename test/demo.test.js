...
test('launch demo renders a zero-network proof card', async () => {
  ...
  assert.equal(report.aggregate.localRoutes, 2);
  assert.equal(report.aggregate.cloudRoutes, 3);
  assert.equal(report.aggregate.policies.balanced, 3);
  assert.equal(report.aggregate.policies.save, 2);
  ...
  assert.match(output, /local routes/);
  assert.match(output, /cloud routes/);
  ...
  assert.match(markdown, /2\/5 routes to local models/);
  assert.match(markdown, /3\/5 routes to cloud models/);
  assert.match(markdown, /Policy mix was/);
  assert.match(markdown, /balanced:3, save:2/);
  assert.match(markdown, /Classifier mix was builtin:5/);
  assert.match(markdown, /Copy line:/);
  assert.match(markdown, /split 2\/5 local and 3\/5 cloud/);
  ...
  assert.match(svg, /split 2\/5 local and 3\/5 cloud/);
  assert.match(svg, /local routes/);
  assert.match(svg, /cloud routes/);
  assert.match(svg, /overhead/);
  assert.match(svg, /balanced:3, save:2/);
  assert.match(svg, /llama3\.2:3b:2/);
