...
test('agent proxy handles browser preflight and malformed JSON before routing', async () => {
  ...
  try {
    ...
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-runner-up/);
    ...
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-context-window/);
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-context-use-pct/);
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-estimated-cost-usd/);
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-baseline-cost-usd/);
    assert.match(preflight.headers.get('access-control-expose-headers'), /x-proofroute-speedup/);
...
test('agent proxy exposes classifier guard proof as browser-readable headers', async () => {
  ...
  try {
    ...
    assert.equal(response.headers.get('x-proofroute-runner-up'), 'none');
    ...
    assert.equal(response.headers.get('x-proofroute-context-window'), '8192');
    assert.ok(Number(response.headers.get('x-proofroute-context-use-pct')) > 0);
    assert.ok(Number(response.headers.get('x-proofroute-estimated-cost-usd')) > 0);
    assert.ok(Number(response.headers.get('x-proofroute-baseline-cost-usd')) > 0);
    assert.ok(Number(response.headers.get('x-proofroute-speedup')) > 0);
    assert.ok(Number(response.headers.get('x-proofroute-savings-pct')) >= 0);
